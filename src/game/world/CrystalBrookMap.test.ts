...
describe('Crystal Brook map', () => {
  it('keeps its spawn and important destinations inside clear walkable space', () => {
    const targets = [
      ...CRYSTAL_BROOK_MAP.entrances.map(({ id, approach }) => ({ id: `entrance:${id}`, position: approach })),
      ...CRYSTAL_BROOK_MAP.collectableSpots.map(({ id, position }) => ({ id: `collectable:${id}`, position })),
      ...CRYSTAL_BROOK_MAP.npcVisitPoints.map(({ id, position }) => ({ id: `npc:${id}`, position })),
      ...CRYSTAL_BROOK_MAP.secretRoutes.map(({ id, position }) => ({ id: `secret:${id}`, position })),
    ];

    expect(isPointInsideWalkableBounds(CRYSTAL_BROOK_MAP, CRYSTAL_BROOK_MAP.playerSpawn, 42)).toBe(true);
    expect(isPointBlocked(CRYSTAL_BROOK_MAP.playerSpawn, CRYSTAL_BROOK_MAP.colliders, 42)).toBe(false);
  ...
  it('provides several river treasures, stepping stones, a secret route and future NPC visit points', () => {
    ...
    expect(new Set(CRYSTAL_BROOK_MAP.collectableSpots.map(({ itemId }) => itemId)).size).toBeGreaterThanOrEqual(2);
  });
});
