...
describe('Whispering Woods map', () => {
  it('keeps the spawn, landmarks and discoveries reachable through safe walkable space', () => {
    ...
    expect(
      isPointInsideWalkableBounds(WHISPERING_WOODS_MAP, WHISPERING_WOODS_MAP.playerSpawn, 42),
    ).toBe(true);
    expect(
      isPointBlocked(WHISPERING_WOODS_MAP.playerSpawn, WHISPERING_WOODS_MAP.colliders, 42),
    ).toBe(false);
