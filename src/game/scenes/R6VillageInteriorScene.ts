...
function questIsAt(questId: QuestId, stepIndex: number): boolean {
  ...
  return progress.status === 'active' && progress.currentStepId === getQuestStepId(questId, stepIndex);
}
...
export class VillageInteriorScene extends Phaser.Scene {
  ...
  private createWindow(x: number, y: number, accent: number): void {
    this.add
      .rectangle(x, y, 190, 126, 0xbce9f4, 1)
  ...
  private createBakerySet(): void {
    ...
    for (const [x, icon] of [
      ...
      this.add.text(x, 330, icon, { fontFamily: UI_FONT, fontSize: '38px' }).setOrigin(0.5).setDepth(7);
    }
    this.add.rectangle(400, 320, 110, 160, 0x875743, 1).setStrokeStyle(5, 0x684232, 0.9).setDepth(5);
    this.add.text(400, 315, '🔥', { fontFamily: UI_FONT, fontSize: '42px' }).setOrigin(0.5).setDepth(6);
    this.add.rectangle(870, 314, 150, 180, 0xe8b984, 1).setStrokeStyle(5, 0xb97a58, 0.8).setDepth(5);
    this.add.text(870, 288, 'RECIPES', { color: '#704637', fontFamily: UI_FONT, fontSize: '14px', fontStyle: 'bold' }).setOrigin(0.5).setDepth(6);
    this.add.text(870, 335, '📜  📖\n📄  🥄', { fontFamily: UI_FONT, fontSize: '28px', align: 'center' }).setOrigin(0.5).setDepth(6);
  }

  private createStoryHouseSet(): void {
    for (const x of [430, 640, 850]) {
      this.add.rectangle(x, 335, 150, 220, 0x6f8aa0, 1).setStrokeStyle(5, 0x526f86, 0.9).setDepth(5);
      for (let row = 0; row < 3; row += 1) {
        this.add.rectangle(x, 282 + row * 62, 126, 8, 0x435d72, 0.9).setDepth(6);
        this.add.text(x, 258 + row * 62, '📕 📗 📘 📙', { fontFamily: UI_FONT, fontSize: '18px' }).setOrigin(0.5).setDepth(6);
      }
    }
    this.add.ellipse(640, 515, 380, 118, 0xf2d7a7, 1).setStrokeStyle(4, 0xb58c60, 0.75).setDepth(5);
    this.add.text(640, 505, '🗺️   📖   ✨', { fontFamily: UI_FONT, fontSize: '34px' }).setOrigin(0.5).setDepth(6);
  }

  private createThreadSet(): void {
    this.add.rectangle(640, 350, 430, 120, 0xb77baa, 1).setStrokeStyle(5, 0x8e5d86, 0.9).setDepth(6);
    for (const [x, icon] of [
      [515, '🎀'],
      [600, '🌸'],
      [685, '✨'],
      ...
      this.add.text(x, 300, icon, { fontFamily: UI_FONT, fontSize: '30px' }).setOrigin(0.5).setDepth(7);
    }
    this.add.text(640, 405, 'Adventure stock changes as the valley grows', {
      color: '#684c67',
      fontFamily: UI_FONT,
      fontSize: '15px',
      fontStyle: 'bold',
  ...
  private renderBakery(): void {
    ...
    const sprite = createSupportingResidentSprite(this, maple).setPosition(1010, 430).setDisplaySize(142, 123);
    const name = this.add.text(1010, 500, 'Maple', {
      color: '#704637', fontFamily: UI_FONT, fontSize: '16px', fontStyle: 'bold', backgroundColor: '#fff3d7e8', padding: { x: 8, y: 4 },
    }).setOrigin(0.5);
    ...
    bakery.listStock().forEach((stock, index) => {
      ...
      const card = this.add.rectangle(x, 250, 285, 150, 0xfffbf3, 0.97).setStrokeStyle(3, 0xd88b62, 0.9);
      const icon = this.add.text(x - 105, 220, stock.definition.icon ?? '🥐', { fontFamily: UI_FONT, fontSize: '34px' }).setOrigin(0.5);
      const title = this.add.text(x - 70, 205, stock.definition.name, { color: UI_COLOURS.ink, fontFamily: UI_FONT, fontSize: '16px', fontStyle: 'bold' });
      const detail = this.add.text(x - 70, 232, stock.isUnlocked ? (stock.isOwned ? 'Owned ✓' : `${stock.price} Shimmer`) : (stock.unlockHint ?? 'Locked'), {
        color: UI_COLOURS.softInk, fontFamily: UI_FONT, fontSize: '12px', wordWrap: { width: 170 },
      });
    ...
    this.createActionButton({ x: 310, y: 485, width: 230, label: '💬 Talk to Maple', onPress: () => this.talkToMaple() }, this.body);
    this.createActionButton({ x: 570, y: 485, width: 230, label: '🎂 Wobbly Cake Plan', onPress: () => this.openCakePlan() }, this.body);

    if (this.shouldShowBakeryMapCorner()) {
      this.createActionButton({ x: 830, y: 485, width: 230, label: '🔎 Check recipe shelf', fill: UI_COLOURS.mint, onPress: () => this.findBakeryMapCorner() }, this.body);
    } else {
      this.createActionButton({ x: 830, y: 485, width: 230, label: '📜 Browse recipes', fill: UI_COLOURS.mint, onPress: () => this.showFeedback('Maple’s recipe shelf contains berry buns, cloud biscuits and one page simply labelled “TRY MORE SPRINKLES”.') }, this.body);
    }
  }

  private renderStoryHouse(): void {
    const tansy = supportingResident('resident:tansy');
    const sprite = createSupportingResidentSprite(this, tansy).setPosition(1015, 430).setDisplaySize(142, 123);
    const name = this.add.text(1015, 500, 'Tansy', {
      color: '#4f6171', fontFamily: UI_FONT, fontSize: '16px', fontStyle: 'bold', backgroundColor: '#eff8ffe8', padding: { x: 8, y: 4 },
    }).setOrigin(0.5);
    this.body?.add([sprite, name]);
    ...
    this.createActionButton({ x: 300, y: 465, width: 230, label: '💬 Talk to Tansy', onPress: () => this.talkToTansy() }, this.body);
    this.createActionButton({ x: 560, y: 465, width: 230, label: '📖 Read story card', onPress: () => this.readStoryCard() }, this.body);
    this.createActionButton({ x: 820, y: 465, width: 230, label: '🗺️ Check valley clues', fill: UI_COLOURS.mint, onPress: () => this.showFeedback(storyHouse.getCurrentClue()) }, this.body);
  }
  ...
  private renderThreadShop(): void {
    ...
    const panel = this.add.text(GAME_WIDTH / 2, 235, `${unlocked}/${stock.length} stock lines unlocked  •  ${owned} wearable${owned === 1 ? '' : 's'} owned\nStarter treasures are affordable now; new pieces appear through stories, discoveries and racing.`, {
      color: UI_COLOURS.ink,
      fontFamily: UI_FONT,
      fontSize: '17px',
      fontStyle: 'bold',
    ...
    this.createActionButton({ x: 470, y: 475, width: 300, label: '🎀 Browse Twinkle & Thread', onPress: () => this.openThreadShop() }, this.body);
    this.createActionButton({ x: 810, y: 475, width: 300, label: '✨ What unlocked?', fill: UI_COLOURS.mint, onPress: () => this.showThreadProgress(stock) }, this.body);
  }

  private createActionButton(options: ActionButtonOptions, parent: Phaser.GameObjects.Container | null): void {
    const fill = options.enabled === false ? UI_COLOURS.lavender : (options.fill ?? UI_COLOURS.gold);
    const button = this.add.rectangle(options.x, options.y, options.width ?? 220, 54, fill, options.enabled === false ? 0.6 : 1)
      .setStrokeStyle(3, options.enabled === false ? UI_COLOURS.lavenderStrong : UI_COLOURS.goldStrong, 0.95)
      .setDepth(16);
    const label = this.add.text(options.x, options.y, options.label, {
      color: UI_COLOURS.ink, fontFamily: UI_FONT, fontSize: '15px', fontStyle: 'bold', align: 'center',
  ...
  private talkToMaple(): void {
    ...
    if (progress.status === 'active' && progress.currentStepId === getQuestStepId(MAPLE_CAKE_QUEST_ID, 0)) {
      engine.notifyCharacterTalked(MAPLE_CHARACTER_ID);
      this.showFeedback('Maple: “I need a celebration cake with personality. Pick a colour plan, then we can wobble it together!”');
    } else if (progress.status === 'active' && progress.currentStepId === getQuestStepId(MAPLE_CAKE_QUEST_ID, 4)) {
      engine.notifyCharacterTalked(MAPLE_CHARACTER_ID);
      this.showFeedback('Maple: “It is magnificently wobbly. That makes it ours. I saved a picnic-basket pattern for you too!”');
    } else if (progress.status === 'completed') {
      this.showFeedback('Maple: “The Wobbly Cake Plan is officially a success. I am still voting for extra sprinkles next time.”');
    } else {
  ...
  private openCakePlan(): void {
    ...
    if (!questIsAt(MAPLE_CAKE_QUEST_ID, 1)) {
      if (progress.status === 'completed') {
        this.showFeedback('Maple’s first Wobbly Cake is already part of Village history. The repeatable baking table comes later in R6.5.');
      } else {
    ...
    const shade = this.add.rectangle(GAME_WIDTH / 2, GAME_HEIGHT / 2, GAME_WIDTH, GAME_HEIGHT, 0x493c50, 0.62).setInteractive();
    const card = this.add.rectangle(GAME_WIDTH / 2, GAME_HEIGHT / 2, 760, 330, 0xfffbef, 1).setStrokeStyle(6, 0xe29b68, 1);
    const title = this.add.text(GAME_WIDTH / 2, 245, 'Pick a Wobbly Cake design', {
      color: UI_COLOURS.ink, fontFamily: UI_FONT, fontSize: '28px', fontStyle: 'bold',
    }).setOrigin(0.5);
    const note = this.add.text(GAME_WIDTH / 2, 285, 'This is the story-sized decorating hook. WP14 can reuse it for the full repeatable baking activity.', {
      color: UI_COLOURS.softInk, fontFamily: UI_FONT, fontSize: '13px', align: 'center', wordWrap: { width: 620 },
    }).setOrigin(0.5);
  ...
  private createOverlayChoice(parent: Phaser.GameObjects.Container, x: number, y: number, labelText: string, onPress: () => void): void {
    const button = this.add.rectangle(x, y, 200, 72, UI_COLOURS.gold, 1).setStrokeStyle(4, UI_COLOURS.goldStrong, 1).setInteractive({ useHandCursor: true });
    const label = this.add.text(x, y, labelText, { color: UI_COLOURS.ink, fontFamily: UI_FONT, fontSize: '17px', fontStyle: 'bold' }).setOrigin(0.5).setInteractive({ useHandCursor: true });
  ...
  private finishCakeDesign(theme: MapleCakeTheme): void {
    ...
    gameEventBus.emit('WORLD_FLAG_CHANGED', { flagId: MAPLE_CAKE_SUNSHINE_FLAG, value: theme === 'sunshine' });
    gameEventBus.emit('WORLD_FLAG_CHANGED', { flagId: MAPLE_CAKE_MOONFLOWER_FLAG, value: theme === 'moonflower' });
    gameEventBus.emit('WORLD_FLAG_CHANGED', { flagId: MAPLE_CAKE_RAINBOW_FLAG, value: theme === 'rainbow' });
    new InventoryService(saveService).addItem(WOBBLY_CAKE_ITEM_ID, 1);
    this.showFeedback(`🎂 ${theme === 'sunshine' ? 'Sunny yellow' : theme === 'moonflower' ? 'Moonflower blue' : 'Rainbow bright'} cake complete! Talk to Maple again so she can see your magnificently wobbly design.`);
  ...
  private findBakeryMapCorner(): void {
    ...
    this.showFeedback('🗺️ Map corner found! It had been used as a recipe bookmark and now smells faintly of berry buns.');
  ...
  private talkToTansy(): void {
    ...
    if (progress.status === 'active' && progress.currentStepId === getQuestStepId(TANSY_MAP_QUEST_ID, 0)) {
      engine.notifyCharacterTalked(TANSY_CHARACTER_ID);
      this.showFeedback('Tansy: “Three corners escaped from my favourite map. One likes notices, one smells like baking, and one flew somewhere sunny.”');
    } else if (progress.status === 'active' && progress.currentStepId === getQuestStepId(TANSY_MAP_QUEST_ID, 5)) {
      engine.notifyCharacterTalked(TANSY_CHARACTER_ID);
      this.showFeedback('Tansy: “They fit! The valley has corners again. I am pinning this map down with four bookmarks this time.”');
    } else if (progress.status === 'completed') {
      this.showFeedback('Tansy: “The repaired map is staying right here. Unless a very determined breeze learns to read.”');
    } else {
  ...
  private readStoryCard(): void {
    ...
    if (cards.length === 0) {
      this.showFeedback('No story cards have reached the shelves yet. Exploring the valley will change that.');
      return;
    }
  ...
  private showThreadProgress(stock: ReturnType<ShopService['listStock']>): void {
    ...
    this.showFeedback(nextLocked ? `Next locked treasure: ${nextLocked.definition.name}. ${nextLocked.unlockHint ?? 'Keep exploring to reveal it.'}` : 'Every current Twinkle & Thread stock line is unlocked. More region-themed stock can join later in R6.5.');
  }
