...
export class PipEggHatchScene extends Phaser.Scene {
  ...
  public create(): void {
    ...
    this.add.rectangle(GAME_WIDTH / 2, GAME_HEIGHT / 2 + 35, 1040, 650, 0xfff2df, 0.98)
    ...
    const egg = this.add.ellipse(GAME_WIDTH / 2, 430, 190, 245, 0xf3e7d0, 1)
      .setStrokeStyle(8, 0xb899c8, 1)
      .setDepth(3);
    const marks = this.add.text(GAME_WIDTH / 2, 430, '✦  ☾  ✦', {
      color: '#b58bc7',
      fontFamily: 'system-ui, sans-serif',
      fontSize: '38px',
      fontStyle: 'bold',
    }).setOrigin(0.5).setDepth(4);
    const title = this.add.text(GAME_WIDTH / 2, 155, 'The strange egg is hatching!', {
      color: '#634f70',
      fontFamily: 'system-ui, sans-serif',
      fontSize: '38px',
      fontStyle: 'bold',
    ...
    const instruction = this.add.text(GAME_WIDTH / 2, 790, 'The shell is wobbling...', {
      color: '#6e5879',
      fontFamily: 'system-ui, sans-serif',
      fontSize: '25px',
      align: 'center',
    ...
    this.time.delayedCall(1250, () => {
      ...
      this.add.text(GAME_WIDTH / 2 - 63, 455, '◜', {
        color: '#efe0c8', fontFamily: 'system-ui, sans-serif', fontSize: '150px', fontStyle: 'bold',
      }).setOrigin(0.5).setAngle(-22).setDepth(3);
      this.add.text(GAME_WIDTH / 2 + 66, 455, '◝', {
        color: '#efe0c8', fontFamily: 'system-ui, sans-serif', fontSize: '150px', fontStyle: 'bold',
      }).setOrigin(0.5).setAngle(22).setDepth(3);
      ...
      this.add.circle(GAME_WIDTH / 2 + 28, 398, 7, 0x493d67, 1).setDepth(7);
      this.add.text(GAME_WIDTH / 2, 350, '✦', {
        color: '#fff3a8', fontFamily: 'system-ui, sans-serif', fontSize: '34px', fontStyle: 'bold',
      }).setOrigin(0.5).setDepth(7);
      this.add.text(GAME_WIDTH / 2, 535, 'Luma', {
        color: '#5f4c78', fontFamily: 'system-ui, sans-serif', fontSize: '34px', fontStyle: 'bold',
        backgroundColor: '#fff9eddd', padding: { x: 14, y: 7 },
      }).setOrigin(0.5).setDepth(8);
      this.tweens.add({ targets: [lumaBody, lumaBelly], y: '-=8', duration: 760, yoyo: true, repeat: -1, ease: 'Sine.InOut' });
      glow.setAlpha(0.34);
      title.setText('Hello, Luma!');
      instruction.setText('Your first companion has hatched.\nPress Interact or tap to welcome Luma home.');
      this.readyToContinue = true;
    });

    this.pointerInput = new PointerTouchInputAdapter();
    this.inputController = new InputController([new KeyboardInputAdapter(this), this.pointerInput]);
    const tapZone = this.add.zone(GAME_WIDTH / 2, GAME_HEIGHT / 2, GAME_WIDTH, GAME_HEIGHT).setInteractive();
