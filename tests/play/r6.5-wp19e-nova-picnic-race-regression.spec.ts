...
const MARIGOLD_APPROACH = { x: 1080, y: 920 } as const;
...
function visiblePanelY(scene: DiagnosticScene): number {
  const panelY = scene.objects.find(
    (object) => object.name === 'dialogue-production-panel' && object.visible,
  )?.y;
  if (panelY === undefined) {
    throw new Error('Expected a visible dialogue panel.');
  }
  return panelY;
}
...
test('Marigold and Nova dialogue keep accepted sizing and Meet Nova works when Nova is already at the picnic', async ({
  ...
  expect(visiblePanelY(meadowAfterMeet)).toBe(ordinaryLinePanelY);
