...
test('H2.8 fresh-game starters support place, replace, move, remove and persistence', async ({
  ...
  expect(editor.objects.some(({ name }) => name === 'cottage-decoration-filter:all')).toBe(true);
  expect(
    editor.objects.some(({ name }) => name === 'cottage-decoration-filter:flowers-plants'),
  ).toBe(true);
  expect(editor.objects.some(({ name }) => name === 'cottage-decoration-filter:hangings')).toBe(
    true,
  );
  expect(
