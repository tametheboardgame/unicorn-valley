...
- `catalogueBlurb` is the compact card copy shown in the browse grid. Keep it to **80 characters or fewer**, ideally one short sentence. This limit is intentionally sized to leave room for two-line titles, metadata badges and the progress footer on the current card layout. New stories should provide it explicitly. Older content without it receives a word-boundary fallback derived from `description`, so full descriptions never have to be visually clipped inside cards.
