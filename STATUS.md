...
State: **technically corrected; candidate-bound smoke and David's visual approval pending**. Final four-class and focused long-name/transition/marking evidence is under `docs/evidence/wp19c-current-head/`. Exact code-head CI `34454561329` passed Validate, compatibility and all three browser shards. Bounded reproduction corrected shard 2's remaining retired-label test; shard 1 passed unchanged on the correction head, so its prior inaccessible failure was not waived or papered over. Candidate-bound smoke `34455924827` correctly selected `3b9058b` and `https://b115c806.unicorn-valley.pages.dev`, then exposed the same retired hidden-input assumption in the deployment journey. That test now uses the sole keyboard pencil; a replacement immutable binding remains required. David's visual approval remains pending. No merge, production or WP19D.
