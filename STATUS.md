...
State: **technically corrected; candidate-bound smoke and David's visual approval pending**. Final four-class and focused long-name/transition/marking evidence is under `docs/evidence/wp19c-current-head/`. Exact code-head CI `34454561329` passed Validate, compatibility and all three browser shards. Bounded reproduction corrected shard 2's remaining retired-label test; shard 1 passed unchanged on the correction head, so its prior inaccessible failure was not waived or papered over. New-style explicit save/reload/Continue coverage passes locally. Immutable preview `https://b115c806.unicorn-valley.pages.dev` represents exact code head `3b9058b`; qualification metadata now binds the supported smoke workflow to that candidate. Candidate smoke and David's visual approval remain pending. No merge, production or WP19D.
