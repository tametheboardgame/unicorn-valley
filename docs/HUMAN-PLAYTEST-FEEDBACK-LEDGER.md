...
- Save-read failure and false purchase success on storage failure: IMPLEMENTED / LATEST-HEAD CI VERIFICATION PENDING in WP19A. Focused tests cover denied reads, checkpoint-less primary failure, throwing post-commit listeners and the three-Shimmer/Berry-Bun failed-write/retry order. David confirmed on the preceding preview that the Berry Bun and unicorn edits persisted after reload; this is normal-use evidence, not fault injection. The first denied-getter browser run exposed boot-manager reads that prevented Title rendering; commit `8a6d35e` fixes them and the complete save-recovery browser spec now passes locally.
