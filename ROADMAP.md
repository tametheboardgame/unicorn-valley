...
11. **H3.11 - Village interiors and local content maturity:** umbrella programme. Rebuild Sunbeam interiors as proper playable world spaces using the same broad physicality philosophy as Moonflower Cottage: the player enters as their unicorn, moves around, collides with walls/furniture/counters, layers correctly around props, approaches NPCs/interactables through the shared interaction system and exits through a physical door. The same NPC must not independently exist outside and inside at once. Role-specific presentation is encouraged, and the Bakery baker must have an unmistakable baker/chef identity including a proper hat and workwear.
    ...
    - **H3.11.2 - Sunbeam Bakery rebuild:** **complete / human-approved 2026-09-23.** Fully walkable bakery with dedicated baker Cinnamon, polished physical displays, proper doorway, category-based shopping UI, finite morning-restocked stock, reusable shop-state architecture and preserved Maple/Wobbly Cake progression.
    - **H3.11.3 - Twinkle & Thread rebuild:** **complete / human-approved 2026-09-23.** Walkable fashion/accessory boutique with dedicated shopkeeper Velvet, deliberate asymmetric circulation, physical racks/displays/style desk, in-room rounded commerce UI, preserved progression-gated economy, full Shop/Talk interaction and a dressing-mirror wardrobe that equips owned shop wearables through the canonical unicorn appearance/save pipeline.

    - **H3.11-R1 - Bakery/Picnic progression and village-NPC remediation:** **complete / human-approved 2026-09-25.** Marigold's picnic progression is gated behind Wobbly Cake completion, Maple remains available at the Bakery, duplicate/legacy Maple and Marigold positions are removed, Marigold is clear of the fountain interaction, and the temporary Wobbly Cake interface overlap is cleaned up. The full baking mini-game remains reserved for H3.11-R2.
    - **H3.11.4 - Story House rebuild and long-form library system:** **active / final polish.** Continue the A-F library/reader programme, with H3.11.4F-R2 as the last bounded UI/layout remediation before Story House human approval.
    - **H3.11-R2 - Wobbly Cake baking mini-game:** start only after H3.11.4 is complete and human-approved, before H3.11.5. Replace the current Wobbly Cake selector with a proper responsive baking mini-game while preserving quest/save/reward compatibility.
    - **H3.11.4 - Story House rebuild and long-form library system:** **active.** Story House is no longer limited to short adventure cards; it becomes a durable in-game library capable of holding many authored stories from short illustrated tales through near-novel-length books while remaining a coherent walkable place in Sunbeam. H3.11.4 uses lettered internal checkpoints so the already-approved H3.11.5-H3.11.10 sequence remains untouched.
      - **H3.11.4A - Physical Story House and storykeeper:** walkable library room, continuous back-wall shelving, clue/records area, story corner, correct collision/layering, dedicated storykeeper Quill and physical interactions. The physical-room foundation was human-approved on 24 September 2026 and is the preservation baseline for later reader work.
      - **H3.11.4B - Library catalogue and content architecture:** **complete / technical foundation accepted 2026-09-24.** Define a content-driven story package with lightweight catalogue metadata, cover references, series/tags, chapter ordering and publication state. Long-form prose lives as separate content assets rather than being compiled into the main game JavaScript. Longer works are split into chapters and loaded only when opened. Adding a conforming story should primarily be a content operation, not new gameplay code. Checkpoint 1 establishes the generated catalogue, per-story manifests, Markdown chapter assets, stable block markers and demand-loaded StoryLibraryService.
      - **H3.11.4C - Reader engine:** **complete / human-approved 2026-09-24.** Build a large Unicorn Valley-styled reader over the walkable room, using browser/DOM text layout rather than forcing long-form prose through Phaser. Support long reading sessions, chapter navigation, responsive reflow, touch/mouse/keyboard operation, adjustable text size and line spacing, readable typography and a clear return to the Story House without losing game state. Checkpoint 1 connects the round story table and Quill to the real 4B catalogue, opens the selected story/chapter through a game-styled DOM reader, keeps the Phaser room underneath, locks world movement while reading and introduces responsive text-size/line-spacing controls plus chapter navigation.
      - **H3.11.4D - Reading persistence and progress:** **complete / human-approved 2026-09-24.** Checkpoint 1 adds canonical save-schema v9 reading state, stable per-book semantic bookmarks, global reader preferences, Continue Reading/resume, progress/completion presentation and focused migration/service coverage. Save an independent reading position for every story, expose Continue Reading, chapter/book completion and library-level completion statistics, and persist reader preferences. Bookmarks must use stable story/chapter/content-block identifiers plus local progress rather than fragile absolute character offsets or page numbers, so ordinary story edits do not destroy a player's place.
      - **H3.11.4E - Illustration pipeline:** **human-approved 2026-09-24; technical closeout running.** Checkpoint 1 established real cover/media assets and generic lazy illustration support. David's visual review showed that a flowing article is the wrong presentation model for a page-led picture book. Checkpoint 2 therefore adds a reusable `paged-picture-book` reading mode while preserving the approved flowing reader for novels/long-form work. *The Duck Bread Baker* now maps PDF story pages 2-42 to 41 demand-loaded reader pages, each containing exactly one matching extracted illustration and its source text, with Previous page / Next page navigation, page numbering and exact page resume through the existing stable chapter/block save contract. Support cover art plus inline/full-width story illustrations without making the whole library part of initial game load. Use web-appropriate compressed image assets, responsive source sizes/thumbnails and lazy loading so only the current catalogue viewport/story/page pulls required artwork.
      - **H3.11.4F - Library UX, Quill integration, scalable discovery and real-content onboarding:** **active / final polish.** Checkpoint 1 establishes metadata-driven discovery/provenance and the scalable browse surface: structured format/genre/audience/length metadata, separate text/illustration/edition rights provenance, title/author/category search, dynamically generated facet filters and player-facing metadata badges without hard-coded category lists. Turn the catalogue into a repeat-use destination with covers, title/series browsing, recently read, unread/in-progress/completed states, Continue Reading and completion stats. Add metadata-driven title/author search, category/filter controls and derived shelves so the library remains usable if it grows to hundreds of works. Extend story metadata with format, genre, audience/reading style, length, series, author and source/rights dimensions; support provenance for original/public-domain/licensed works, including the ability to distinguish text rights from illustration/edition rights where necessary. Quill and physical Story House interactions should use the same authoritative catalogue, taxonomy and reader flow so recommendations are metadata-driven rather than bespoke. Preserve and integrate the existing short Valley adventure/story cards as appropriate while onboarding multiple real authored stories to prove that the pipeline works without per-book code changes. **H3.11.4F-R2 - final Story House UI polish:** compact the blue library/reader header areas to recover vertical reading/browsing space; clamp/contain long book descriptions and completion/progress copy so card content never escapes its border; pin progress to a stable card footer; and make Previous/Next reader buttons equal in width while preserving the now-approved collapsible Search/Filters behaviour. **H3.11.4F-R3 - browse-space and catalogue-copy polish:** move Continue Reading and Valley Story Cards out of the scrolling book grid into compact top-level library actions/views so the scrollable catalogue is reserved for books; introduce an explicit short catalogue blurb for future stories with a concise length contract, plus a safe generated fallback for older content, so book cards remain readable without raw clipping; preserve the approved equal reader navigation and collapsible Search/Filters behaviour.
      - **H3.11.4 architecture/performance contract:** catalogue metadata may be loaded cheaply, but full chapters, large covers and illustrations must not all be bundled/preloaded at game startup. Story text should remain lightweight, chapter-addressable content; image weight is handled through optimisation and lazy loading. Reader presentation should feel native to Unicorn Valley even though the browser DOM owns long-form text layout. Existing saves, quests and StoryHouseService progression remain compatible unless a bounded migration is explicitly required and tested.
      - **H3.11.4 acceptance:** Story House must support a growing collection of short, long and heavily illustrated stories; let the player browse, read for extended periods, resize text, leave and return, resume several different books independently, see useful progress/completion state and do all of this without materially degrading normal game startup or requiring bespoke code for each new story.
